...
const SettingsMenu: React.FC<ISettingsMenuProps> = (props: ISettingsMenuProps) => {
    ...
    const handleClickSyncNow = async () => {
        ...
        const threshold = typeof(props.lastSolved?.timestamp) === "number" ? props.lastSolved.timestamp : twoHoursAgo
        const filtered: RestSubmission[] = submissionsJSON?.filter(
            (submission: RestSubmission) => {
                return submission.timestamp > threshold &&
                submission.title_slug != props.lastSolved?.slug
            }
        )
        console.log(filtered)
        chrome.runtime.sendMessage({
            type: "process-submissions",
