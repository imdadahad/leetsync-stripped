...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    if (request.type === "process-submissions") {
        processSubmissionsInBackground(request.submissions)
        sendResponse({ status: "started" })
    }
  });

  async function processSubmissionsInBackground(submissions: RestSubmission[]) {
    for (const submission of submissions) {
        console.log(`attempting to submit: ${submission}`)
      try {
            const leetcodeHandler = new LeetCodeHandler()
            const github = new GithubHandler()
            const details = await leetcodeHandler.getSubmission(submission.title_slug)
            if (details) {
                console.log(`details: ${details}`)
                await github.submit(details)
            }
            setTimeout(() => 500)
        } catch (e) {
